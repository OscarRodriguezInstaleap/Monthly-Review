...
def apply_filters(df: pd.DataFrame) -> pd.DataFrame:
    ...
    with st.sidebar:
        ...
        # Filtro por clientes (grupos seleccionados)
        clients = sorted([c for c in df["cliente"].dropna().unique() if c and c != "None"])
        sel_clients = st.multiselect("Clientes (opcional)", clients, default=[])
        if sel_clients:
            df = df[df["cliente"].isin(sel_clients)]

        # Top-N por último MRR
        st.markdown("---")
...
def kpis_overview(df: pd.DataFrame):
    ...
    # Extended KPIs
    ext = compute_kpis_extended(df)
    if ext:
        c5,c6 = st.columns(2)
        c5.metric("ARR (run-rate)", f"${ext['ARR']:,.0f}")
        nrr_val = ext.get('NRR_yoy', None)
        nrr_txt = f"{nrr_val:,.1f}%" if nrr_val is not None else "—"
        meta = ext.get('NRR_yoy_meta', {})
        base_month = meta.get('base_month', None)
        foot = f"Cohorte basada en {base_month.strftime('%b %Y')}" if isinstance(base_month, pd.Timestamp) else ""
        c6.metric("NRR YoY (cohorte)", nrr_txt, help=foot)
        with st.expander("Top crecimientos/decrecimientos (YoY, por cliente)"):
            colA, colB = st.columns(2)
            tg = ext['top_growth'].copy(); tg['diff'] = tg['diff'].round(0)
            td = ext['top_decline'].copy(); td['diff'] = td['diff'].round(0)
            colA.write("**Top crecimiento** (Δ vs mismo mes año previo)")
            colA.dataframe(tg, use_container_width=True)
            colB.write("**Top decrecimiento** (Δ vs mismo mes año previo)")
            colB.dataframe(td, use_container_width=True)

def cohort_clients(m_df: pd.DataFrame, month: pd.Timestamp) -> List[str]:
    sel = m_df[(m_df["date"]==month) & (m_df["value"]>0)]
    return sorted(sel["cliente"].dropna().unique().tolist())

def compute_kpis_extended(df: pd.DataFrame) -> Dict[str, Any]:
    out: Dict[str, Any] = {}
    m = df[df["metric"]=="MRR"].copy()
    if m.empty:
        return out
    max_date = m["date"].max()
    yoy_date = (max_date - pd.DateOffset(years=1))

    total_last = m[m["date"]==max_date]["value"].sum()
    out["ARR"] = total_last * 12.0

    if not pd.isna(yoy_date):
        cohort = m[(m["date"]==yoy_date) & (m["value"]>0)]["cliente"].unique().tolist()
        base = m[(m["date"]==yoy_date) & (m["cliente"].isin(cohort))]["value"].sum()
        now_for_cohort = m[(m["date"]==max_date) & (m["cliente"].isin(cohort))]["value"].sum()
        out["NRR_yoy"] = (now_for_cohort / base * 100.0) if base > 0 else None
        out["NRR_yoy_meta"] = {"cohort_size": len(cohort), "base_month": yoy_date}
    else:
        out["NRR_yoy"] = None
        out["NRR_yoy_meta"] = {"cohort_size": 0, "base_month": None}

    g_last = m[m["date"]==max_date].groupby("cliente", as_index=False)["value"].sum().rename(columns={"value":"last"})
    g_yoy = m[m["date"]==yoy_date].groupby("cliente", as_index=False)["value"].sum().rename(columns={"value":"yoy"})
    movers = g_last.merge(g_yoy, on="cliente", how="outer").fillna(0.0)
    movers["diff"] = movers["last"] - movers["yoy"]
    out["top_growth"] = movers.sort_values("diff", ascending=False).head(10)
    out["top_decline"] = movers.sort_values("diff", ascending=True).head(10)
    out["max_date"] = max_date
    out["yoy_date"] = yoy_date
    return out
...
def make_forecast(df: pd.DataFrame, metric: str, horizon: int, aggregate_first: bool, by: Optional[List[str]]=None) -> Tuple[pd.DataFrame, pd.DataFrame]:
...
def yoy_cohort_bar(df: pd.DataFrame):
    m = df[df["metric"]=="MRR"].copy()
    if m.empty:
        st.caption("No hay datos de MRR para gráfico YoY por cohorte.")
        return
    all_months = sorted(m["date"].dropna().unique().tolist())
    sel_month = st.selectbox("Selecciona el mes a comparar (cohorte YoY)", options=all_months, index=len(all_months)-1, format_func=lambda d: d.strftime("%b %Y"))
    yoy_month = sel_month - pd.DateOffset(years=1)

    cohort = cohort_clients(m, sel_month)
    total_now = m[(m["date"]==sel_month) & (m["cliente"].isin(cohort))]["value"].sum()
    total_yoy = m[(m["date"]==yoy_month) & (m["cliente"].isin(cohort))]["value"].sum()

    fig = go.Figure()
    label = "Cohorte " + sel_month.strftime("%b %Y")
    fig.add_trace(go.Bar(x=[label], y=[total_now], name=sel_month.strftime("%b %Y")))
    fig.add_trace(go.Bar(x=[label], y=[total_yoy], name=yoy_month.strftime("%b %Y")))
    fig.update_layout(barmode="group", title="MRR por cohorte (clientes activos en el mes seleccionado) vs mismo mes del año anterior",
                      xaxis_title="", yaxis_title="MRR", height=420, margin=dict(l=10,r=10,t=40,b=10))
    st.plotly_chart(fig, use_container_width=True)

    with st.expander("Detalle por cliente (diferencias YoY en el mes seleccionado)"):
        det_last = m[(m["date"]==sel_month) & (m["cliente"].isin(cohort))].groupby("cliente", as_index=False)["value"].sum().rename(columns={"value":"valor_mes"})
        det_yoy = m[(m["date"]==yoy_month) & (m["cliente"].isin(cohort))].groupby("cliente", as_index=False)["value"].sum().rename(columns={"value":"valor_yoy"})
        det = det_last.merge(det_yoy, on="cliente", how="outer").fillna(0.0)
        det["diff"] = det["valor_mes"] - det["valor_yoy"]
        st.dataframe(det.sort_values("diff", ascending=False), use_container_width=True)
...
# KPIs (incluye ARR, NRR, Top movers YoY)
...
# Transactions section (opcional pronóstico)
...
st.caption("Consejo: usa Excel con hojas 'MRR', 'CIHS' y 'Transactions' para mejores correlaciones y alertas.")
