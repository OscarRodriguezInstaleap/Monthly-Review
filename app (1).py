...
def build_unified_long(tables: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    if parts:
        ...
        # Backfill meta per cliente (zona/type/nuevo/pais) from last non-null seen
...
def filters_ui(df: pd.DataFrame):
    ...
    with st.container():
        ...
        # Clientes (depende de zona+tipo+nuevo)
    ...
    for client, g in m.groupby("cliente"):
        ...
        if risk or signals:
            alerts.append({"cliente": client, "zona": g["zona"].iloc[-1] if "zona" in g.columns else None,
                           "type": g["type"].iloc[-1] if "type" in g.columns else None, "pais": g["pais"].iloc[-1] if "pais" in g.columns else None,
...
def map_by_country(unified: pd.DataFrame):
    ...
    st.caption("Sugerencia: usa zoom y pan del mapa para explorar. Haz clic en un país para ver el detalle.")
    ...
    plot_series_with_forecast(hist_df, fcst_df, f"{sel_country} - MRR Histórico + Pronóstico")
...
# Pronóstico por cliente si hay filtro de clientes
if selected_clients:
...
# Transacciones (siempre en base a filtros)
...
# Mapa por país (nuevo)
map_by_country(unified=filtered)
