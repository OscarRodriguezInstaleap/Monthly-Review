...
def standardize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    mapping = {"type client":"type","tipo cliente":"type","tipo de cliente":"type","nuevo":"nuevo","zona":"zona",
               "cliente":"cliente","client":"cliente","razon social":"razon_social","razón social":"razon_social",
               "razon_social":"razon_social","pais":"pais","país":"pais","country":"pais"}
...
def detect_meta_and_time(df: pd.DataFrame) -> Tuple[List[str], List[str]]:
    def is_time_header(col):
        ...
        s = str(col).strip()
        month_regex = re.compile(r"^(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec|ene|feb|mar|abr|may|jun|jul|ago|sept|sep|oct|nov|dic)[a-z]*[-_/ ]?\d{2,4}$", re.IGNORECASE)
        if month_regex.match(s): return True
...
def to_long(df: pd.DataFrame, metric_name: str) -> pd.DataFrame:
    ...
    if not ts_cols: return pd.DataFrame(columns=meta_cols + ["date","value","metric"])
    long_df = df.melt(id_vars=meta_cols, value_vars=ts_cols, var_name="month", value_name="value_raw")
    long_df["value"] = (long_df["value_raw"].astype(str)
                        .str.replace(",","",regex=False).str.replace(" ","",regex=False)
                        .replace({"":"None","nan":"None","None":"None","NULL":"None","null":"None"})).replace("None", pd.NA)
...
def build_unified_long(tables: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for key, df in tables.items():
        ...
        if key_norm in ("mrr","revenue"): parts.append(to_long(df, "MRR"))
        elif key_norm in ("cihs","health"): parts.append(to_long(df, "CIHS"))
        elif key_norm.startswith("trans") or key_norm in ("tx","orders","transactions"): parts.append(to_long(df, "Transactions"))
        else: parts.append(to_long(df, key))
    if not parts:
    ...
    for col in meta_cols:
        out[col] = out[col].fillna(out[f"{col}_bf"]); out.drop(columns=[f"{col}_bf"], inplace=True)
