...
def filters_ui(df: pd.DataFrame):
    ...
    with st.container():
        ...
        zones_all = sorted([z for z in df.get("zona", pd.Series()).dropna().unique() if z])
        ...
        types_all = sorted([t for t in df_z.get("type", pd.Series()).dropna().unique() if t])
        ...
        nuevos_all = sorted([n for n in df_zt.get("nuevo", pd.Series()).dropna().unique() if n])
        ...
        clients_all = sorted([c for c in df_ztn.get("cliente", pd.Series()).dropna().unique() if c])
